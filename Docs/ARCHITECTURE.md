...
**The flow is now two-way between Input Script and analysis (revised — see decisions #26–#29).** The earlier design's claim that "the user is never returned to Input Script after a successful transcription" no longer holds: the analysis screen's leading control is a ‹ back to Input Script in every state, and it returns the user to the *same live step* rather than a rebuilt one. `beginAnalysis` therefore keeps `inputViewModel` alive instead of releasing it, and `path` is set to `[.input, .analysis]` — replacing the loading step rather than appending to it, so ‹ lands on Input Script and not on a spinner the user has already passed.

`confirmInput()` is where ✓ is decided. It finalizes the active mode, and then either returns to the analysis already held for that exact `SpeechSource` (`path = [.input, .analysis]`, no transcription, no generation) or, when the source changed, clears the retained draft and transcribes afresh. Reusing it requires the *view model* to have survived the pop, which `NavigationStack` does not do for state a destination view owns — so `AnalysisViewModelCache` in the app target holds one per draft id, keyed on `ScriptDraft.id`, and the coordinator handing back the same draft is what returns the same screen. The title is the one thing Input Script can still have changed on the way back, so it travels forward through `TranscriptAnalysisViewModel.applyTitle(_:)`; everything else on that screen belongs to the analysis.

Leaving analysis by ✓ still ends the create flow rather than revealing a purpose picker three steps stale, and no presentation is ever stacked, so nothing has to be unwound.
...
**Going back resumes the step; it no longer rebuilds it (revised — decision #27).** `returnToInput(from:)` pops to `[.input]` with `inputViewModel` untouched, so the user finds the mode they left, the take still on screen, and the take still playable. It restores the title from the draft and calls `stashTranscript(_:)`, which parks the transcript in Write mode **without switching to it** and **without overwriting anything already typed** — so a rejected transcript is within reach for a user who goes looking to reword it, which is what a rejection asks them to do, while a user who simply changed their mind gets their recording back rather than a wall of text. A rebuild path survives for the case where there is no live step to resume, seeding Write mode from the transcript as before.

> **Superseded.** `returnToInput(rejecting:)` cleared `analysisDraft` and rebuilt Input Script seeded in Write mode, on the reasoning that "the original session's recorder and file handles are long gone". They are not gone any more: the step is kept alive across analysis precisely so the recording survives, which is what US2's back button asks for.
...
- **A take is written in segments, one per run of recording** *(added — decision #26)*. An AAC file still open for writing has no index and cannot be read back, so `pause()` closes the current `.m4a` and `resume()` opens the next. That is what makes `previewURL()` — replaying a take *mid-session*, without ending it — possible at all; `AudioSegmentMerger` (`AVMutableComposition` + `AVAssetExportSession`) puts the pieces back together for `finish()`, and the ordinary case of a single pause never merges anything because there is only ever one segment. `finish()` adopts an up-to-date preview rather than exporting the same audio twice, which is the common path: pause, listen back, confirm.
- **Replay and Retake** *(added)*. `AudioPlaying` (an `AVAudioPlayer` actor in ShuoAudio) drives an inline play/pause control beside the waveform, with the playhead dimming the bars it has not reached. Retake throws the take away and returns to an empty recorder — no confirmation, matching Attach File's "Reupload File", which is the same gesture one segmented-control tap away. Retake needs a *new* capture session, since `AudioCapturing` is single-use by contract, so `SpeakModeViewModel` is injected with a **factory** rather than an instance.
- **A finished take cannot be recorded into again.** After ✓ its session has ended, so stepping back from analysis offers replay and Retake but no resume — offering a control that could only fail would be worse than offering none.
- **The audio file is deleted by `AudioRecordingDeleting`, not by `discard()`** *(added — decision #28)*. `AudioCapturing.discard()` is a no-op once a session has ended, and by the time v1 wants the take gone — the user has saved or left — that is exactly the state it is in. A separate one-method protocol deletes the file; `AudioRecordingFileStore` implements it.
...
| 18 | Where does a rejected transcript go? | **Back to Input Script — revised by #27 to *resume* the step rather than rebuild it, with the transcript parked in Write mode instead of opening there.** Original reasoning follows. **Back to Input Script, seeded in Write mode.** Re-running the same transcript reaches the same verdict, so ✓ on a rejection is not a retry — it is "edit your draft". Without it the only exit was ✕, which meant re-recording a speech the app already had. This is the single place the flow moves backwards, and it is justified because it is the single place where the earlier step is the actual fix. See §3.1.1. |
...
| 26 | How can a take be replayed while the user is still mid-recording? | **Record in segments: pause closes the file, resume opens the next.** The UI keeps pause/resume *and* gains an inline replay control with no extra button, which was the requirement — but an AAC file open for writing has no index and simply cannot be read back, so "play what you have so far" is impossible without first closing something. Closing on pause is the smallest thing that can be closed. The cost is a take spread across files, paid by `AudioSegmentMerger` at `finish()`, and only when the user actually resumed: one pause means one segment and no export at all. `finish()` also adopts an up-to-date preview instead of exporting the same audio a second time, so the common pause → listen → confirm path merges once, not twice. Rejected: recording LPCM/CAF so a partial file is readable (≈10× the bytes for audio that is deleted at the end of the flow), and finalizing the take on pause (loses resume, which the segmented design keeps). |
...
| 34 | What is an untitled script called? | **`Untitled Script 1`, `Untitled Script 2`, … — numbered highest-plus-one, minted at Input Script, revising #25's flat "Untitled Script".** A library of identically-named scripts is unusable, and with no rename prompt in the flow that was the default outcome. Three choices worth recording. **Numbered from 1, with no bare form**, so there is exactly one format to write and to parse — the old unnumbered placeholder is now just another title a user may type and own. **Highest-plus-one rather than lowest-unused**, so a number freed by a rename or a delete is never issued twice; two scripts that had both been `Untitled Script 3` would be indistinguishable in the library. **Resolved at Input Script, not at save**, so the name in the analysis screen's title field *is* the name that gets saved — resolving silently inside `save()` would show the user one name and store another. The number depends on what is stored, so this needs I/O, while `resolvedTitle` and `commitTitle()` are read synchronously by `makeDraft` and by ✓ straight from the keyboard. Hence `UntitledScriptTitle` (pure format/parse/next, in `ShuoCore` because both Feature packages need it and neither may depend on the other) plus `NextUntitledScriptTitleUseCase`, whose result each view model **caches** — `prepareUntitledPlaceholder()` from the input step's `.task`, `resolveUntitledPlaceholder()` inside the analysis step's existing `start()` task. The failure mode this design exists to prevent: because #16 saves the script the moment classification succeeds, a naive re-resolve on the way back finds *that very record* and numbers past it, renaming the user's script from 1 to 2 for stepping back and confirming again. So a generated name coming back is **adopted, not recomputed** — `restoreTitle` keeps its number, and analysis skips the query entirely when the draft already carries one. Only a script with a name of its own needs a fresh number, which is the reopened-then-cleared case. A failed read leaves `Untitled Script 1` standing rather than blocking the step: the name of an untitled script is not worth failing an analysis over, and a store that broken will report a real error at save time anyway. Matching is strict and case-sensitive (stem, one space, ASCII digits, bounded length so `+ 1` cannot overflow), so `Untitled Script ideas` and `untitled script 40` stay the user's own and never shift the count — the store's `search(query:)` is a loose case-insensitive substring match used only to narrow the fetch, with `UntitledScriptTitle` making the real decision. |
| 35 | Where does ‹ go while ↺ is refining the whole screen? | **Back to the analysis it is covering — not out of it, and not to Input Script.** ↺ takes the screen over with "Refining script…", so `isForceRegenerating` forces ‹ into the toolbar even in states #29 would hide it in. It shared `goBack()` with the ordinary case, which meant a user who only wanted to stop a refinement was sent *two* steps back, out of the analysis and into Input Script. `cancelForceRegeneration()` splits the two: it cancels the run, clears the flag, and puts back the refinement ↺ had already cleared — asking to redo something and then changing your mind should not cost you the version you had. Reopened-from-library scripts appeared correct all along only by accident, having no `onBack` to fall through to; they now take the same explicit path. The accessibility label follows the meaning ("Cancel refining" vs. "Back to input") rather than the glyph. |
| 36 | Why is the mascot GIF decoded off the main thread? | **Because decoding it on the main thread froze the navigation bar for seconds, and the loading screens are exactly where it shows.** `SHUO LOAD` is 165 frames at 1080×1080; `UIImage.animatedImage(with:duration:)` needs all of them decoded up front, and `NativeGifView` did that inline in `makeUIView` — ~1.3 s on an M-series Mac, longer on device, per appearance. A frozen main thread is a frozen `NavigationStack`: ‹ on the transcription screen rendered as an empty glass capsule until the decode finished, and taps aimed at it queued and were delivered afterwards to whatever screen the first tap had already navigated to, which is how spamming it landed the user back on the purpose picker. Decoding now runs detached, one decode per asset shared through `GifImageLoader`'s in-flight table (the transcription and analysis loading screens overlap during the push and ask for the same loop in the same frame), and the result is held in an `NSCache` — the decoded frames are ~145 MB of bitmap, worth keeping while a flow is showing loading screens and worth evicting under memory pressure. Peak memory *drops*, because the old code built a second full set for the incoming screen before the outgoing one was released. Frames are decoded to 480 px, the 160 pt box `LoadingView` gives them at @3x — one constant on the loader rather than a per-call parameter, since the cache is keyed by asset name and a second size would silently be served the first one's frames. Two guards keep it from regressing: `GifContainerView` tracks the asset it has already requested, since SwiftUI calls `updateUIView` on every re-render, and the load task is cancelled on `deinit`. **The decode is also started early rather than on demand.** `PurposeSelectionView.task` calls `LoadingView.prewarmArtwork()`, so the work happens while the user is choosing a purpose and then filling in a script, and the loading screen it feeds is two steps away — by which time the frames are cached and appear in the frame the screen does. It runs at `.utility`, not `.userInitiated`: nothing is waiting on it and it must not compete with a live recording. A loading screen that does appear mid-decode joins the in-flight task rather than starting a second one. |
| 37 | What does a purpose card do when it is tapped, and what does it look like on the way back? | **It fills on touch-down, stays filled through the push, and is idle again when the user returns.** Three changes to one interaction. **The 200 ms wait is gone**: `PurposeSelectionView` held the flow so the tapped card could be seen turning aqua first, which is 200 ms in which a tap appears to have done nothing, on the first screen of the flow and the one where the user has least patience. `navigationTask` and its `onDisappear` cancellation go with it. **The fill now starts on press, not on selection**: `.plain` gives custom content no highlight, so a tap looked identical to no tap until the next screen slid in — long enough on a slow frame to invite a second tap. `PurposeCard` draws itself from inside a private `ButtonStyle`, the only place `isPressed` is observable, and fills for either reason (`isSelected || isPressed`) so the two are indistinguishable. Unanimated deliberately: the release clears `isPressed` in the same update that sets `isSelected`, so the fill hands over without blinking, and any fade would just be something else to wait through. The label is `EmptyView()` because the style draws everything — the fill has to reach the text colours, not only the background — so the card carries an explicit `accessibilityLabel`. **The lasting selection is the coordinator's, not a `@State` copy**: `PurposeSelectionView` reads `coordinator.selectedPurpose`, which `dismissInputScript()` already clears, so ‹ from Input Script returns to three unselected cards for free. A local mirror could only be a second answer to the same question, waiting to disagree with the first — which is exactly what it did, leaving the previous choice filled on the way back. |
...
│                          makeTranscriptAnalysisViewModel(draft:)) that wire concretes
│                          to the protocols each package's inits expect.
├── RootView.swift         Home + the two sheets. Holds AnalysisViewModelCache, which keeps
│                          one analysis view model alive per draft id so ‹ then ✓ returns
│                          to the screen the user left rather than regenerating it (#27).
...
│   │   ├── AudioPlaybackEvent.swift   enum: progress/finished/failed — replay's counterpart (§3.1.3)
...
│   │   ├── UntitledScriptTitle.swift  "Untitled Script N" — format, strict parse, next number (#34)
...
│   │   ├── AudioPlaying.swift             play(url:)/pause()/stop() + event stream — replay (§3.1.3)
│   │   ├── AudioRecordingDeleting.swift   delete(_:) — outlives the capture session (#28)
...
│   │   └── NextUntitledScriptTitleUseCase.swift  names an untitled script, numbered past stored (#34)
│   │
│   └── Errors/
...
│   │   ├── AudioSegmentMerger.swift         joins pause-split segments back into one m4a (#26)
│   │   └── AudioRecordingFileStore.swift    conforms to AudioRecordingDeleting (#28)
│   ├── Playback/
│   │   └── AudioPlaybackService.swift       actor; AVAudioPlayer behind AudioPlaying — polls
│   │                                        the playhead instead of using the delegate
│   ├── Transcription/
...
│   │   ├── PillButton.swift             the quieter action beside it — Retake, Reupload File
│   │   ├── WaveformView.swift           optional 0...1 progress dims the bars past the playhead
│   │   ├── AccordionView.swift
...
└── Sources/ShuoTestSupport/
    ...
    ├── FakeAudioPlaying.swift                actor; records what it was asked to play
    ├── FakeAudioRecordingDeleting.swift      actor; records what it was asked to delete
...
│   ├── Coordinator/
│   │   └── CreateScriptCoordinator.swift    Route enum + path; keeps the input step alive across
│   │                                        analysis and resumes it on ‹ (§3.1.1, #27)
│   ├── Purpose/
...
│   ├── AnalysisToolbarControls.swift       which of ‹ / ✓ this screen offers (#29)
...
└── Tests/FeatureTranscriptAnalysisTests/
    ...
    ├── AnalysisToolbarControlsTests.swift       no state is ever left without a way out
